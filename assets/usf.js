/* USF file - DO NOT MODIFY THIS FILE. THIS FILE IS REGULARLY CHANGED BY USF APP AND **ANY DIRECT CHANGES WILL BE LOST**. Use our in-app customization if you need to update CSS and JS code. Auto modified at: 6/14/2022 8:39:28 AM*/
/* Begin custom theme code */
// define templates for the Kalles 2.7.1 theme
...
<a class="db" :href="productUrl">
    <div class="pr_lazy_img main-img nt_img_ratio lazyloadt4s nt_bg_lz"  :data-bgset="_usfGetDataBgSet(scaledSelectedImageUrl)" data-parent-fit="width" data-wiis :data-ratio="_usfGetImageRatio(selectedImage)" data-sizes="auto" :style="window._usfGlobalSettings.sett_equal ? false:  'padding-top:' + 100/_usfGetImageRatio(selectedImage) + '%;'"></div>
</a>
<div v-if="hoverImage" class="hover_img pa pe_none t__0 l__0 r__0 b__0 op__0">
    <div class="pr_lazy_img back-img pa lazyloadt4s nt_bg_lz"  :data-bgset="_usfGetDataBgSet(scaledHoverImageUrl)" data-parent-fit="width" data-wiis :data-ratio="_usfGetImageRatio(hoverImage)" data-sizes="auto" :style="window._usfGlobalSettings.sett_equal ? false: 'padding-top:' + 100/_usfGetImageRatio(hoverImage) + '%;'"></div>
</div>
<div class="hover_button op__0 tc pa flex column ts__03 lazyloadt4s" 
:class="[{'has_2btnst4_pr': _usfAdditionalSettings.use_quickview && !_usfAdditionalSettings.hide_btns},'des_btns_pr_' + _usfAdditionalSettings.des_btns_pr, 'has_sizelistt4_' + String(hasSize)]"
:data-rendert4s="_usfAddQuery(productUrl,'view=usf-hover-btn&variant=' + selectedVariantForPrice.id)"></div>
`;
var _usfProductPrice = `
<span class="price dib mb__5">
    <template v-if="_usfGlobalSettings.inc_price == 1">
...
var _usfSearchResultsViewsTpl =
    ...
    <div data-view="6" class="usf-view usf-grid usf-flex-view-6" :class="{'usf-active': ((usf.isMobile && _usfGlobalSettings['col_mb'] == 6) || (!usf.isMobile && _usfGlobalSettings['col'] == 6)) && usf.queryRewriter.view != 'list'}" @click="onGridViewClick();usfViews(6)"><svg width="19px" height="19px" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 19 19"><path d="M7,2v5H2V2H7 M9,0H0v9h9V0L9,0z"></path><path d="M17,2v5h-5V2H17 M19,0h-9v9h9V0L19,0z"></path><path d="M7,12v5H2v-5H7 M9,10H0v9h9V10L9,10z"></path><path d="M17,12v5h-5v-5H17 M19,10h-9v9h9V10L19,10z"></path></svg></div>   
...
<div :class="'col-lg-' + _usfGlobalSettings['col'] + ' col-md-' + _usfGlobalSettings['col_tb'] + ' col-' + _usfGlobalSettings['col_mb'] + ' mt__30 pr_grid_item product nt_pr desgin__'+window._usfGlobalSettings['inc_pr']+' usf-grid-item '+(window._usfGlobalSettings['inc_pr'] == 2 ? ' tc':'')" @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave" :data-usf-pid="product.id">
    <div class="product-inner pr">
        <div class="pr_wr_dcimg pr oh" v-if="countdown !='' && window._usfGlobalSettings.use_cdt ">
            <div @click="onItemClick" @mouseover="onItemHover" @mouseleave="onItemLeave"  class="product-image pr oh lazyloadt4s">
                `+ _usfProductImage + `
            ...
            <div class="pr_deal_dt pa pe_none op__0">
                ...
                <span class="pr_coun_dt" :data-date="countdown"></span>
            </div>
        </div>
        
        <div v-else class="product-image pr oh lazyloadt4s" >
            `+ _usfProductImage + `
            <a :href="productUrl" class="usf-action-button" :class="{'usf-hide-sale': !(hasDiscount && usf.settings.search.showSale), 'usf-hide-soldout': !(isSoldOut && usf.settings.search.showSoldOut)}">
                <div  class="product-image pr oh lazyloadt4s" :data-rendert4s="_usfAddQuery(productUrl,'view=no-usf&variant=' + selectedVariantForPrice.id)"> </div>
            </a>
       ...
       <div class="product-info mt__15">
        ...
        <div v-if="colorCount > 0" class="swatch__list_js swatch__list lh__1 nt_swatches_on_grid lazyloadt4s" :data-rendert4s="_usfAddQuery(productUrl,('view=sw'+ window._usfGlobalSettings.sett_equal))"></div>
        <!-- Swatchs-->
...
<nav class="nt-pagination w__100 tc">
    <ul class="pagination-page page-numbers">
        <template v-for="e in elements">
            <li v-if="e.type === 'prev'" class="prev page-numbers">
                <a href="javascript:void(0)" :title="loc.prevPage" @click="onPrev" class="prev page-numbers">←</a>
            </li>
            <li v-else-if="e.type === 'dots'"><span class="page-numbers lh__1">…</span></li>
            <li v-else-if="e.type === 'page' && e.current"><span class="page-numbers current">{{e.page}}</span></li>
            <li v-else-if="e.type === 'page' && !e.current"><a class="page-numbers" href="javascript:void(0)" @click="ev=>onPage(e.page,ev)" :title="usf.utils.format(loc.gotoPage,e.page)">{{e.page}}</a></li>
            <li v-else-if="e.type === 'next'" >
                <a href="javascript:void(0)" :title="loc.nextPage" @click="onNext" class="next page-numbers">→</a>
            </li>
        </template>
    </ul>
</nav>
...
usf.event.add('init', function () {
    ...
    if (!window._usfFromText || window._usfFromText.includes('missing'))
        window._usfFromText = usf.settings.translation.from;

    usfGridWrapperClass = `usf-results products nt_products_holder row fl_center row_pr_${window._usfGlobalSettings['inc_pr']} cdt_des_${window._usfGlobalSettings.cdt_des} round_cd_${window._usfGlobalSettings.round_cd} ${window._usfGlobalSettings.image_size} ${window._usfGlobalSettings.image_ratio} position_${window._usfGlobalSettings.image_position} space_${window._usfGlobalSettings.space} js_isotope nt_isotope`;
    ...
    usf.event.add(["sr_updated", "sr_viewChanged", "rerender"], function () {
        setTimeout(function () {
            if (window.geckoShopify) {
                ...
                geckoShopify.wishlistUpdate();
            }
        }, 1000);
    });

    window._usfAdditionalSettings = window._usfAdditionalSettings || {
        use_quickview : true,
        hide_btns: false,
        des_btns_pr: "2",
    ...
    var NewSearchResultsGridItem = {
        ...
        data(){
            var tooltip = _usfAdditionalSettings.des_btns_pr == '1' ? 'ttip_nt tooltip_top_left' : 'ttip_nt tooltip_top'; 
            var color_count = 0;
            var has_size = false; //<=>ck_s
            var countdown = usf.utils.getMetafield(this.product,'meta','countdown');

            for(let i = 0; i <  this.product.options.length;i++){
                var o = this.product.options[i];
                var name = o.name.toLowerCase();
                if(_usfAdditionalSettings.get_size.includes(name)){
                    has_size = true;
                }
                if(_usfAdditionalSettings.get_color.includes(name) && _usfGlobalSettings.show_color){
                    color_count = o.values.length;
                    if(_usfGlobalSettings.show_cl_type == '2'){
                        var count = 0;
